...
from common.exception import ServerException, ClientException
...
def schema(query=None, form=None, json=None, reply=None, error=None):
    ...
    def _load_query(handler, schema):
        ...
        for key, value in handler.request.query_arguments.items():
            if not key.endswith('[]'):
                data[key] = value[-1].decode(encoding='UTF-8',errors='strict')
            else:
                data[key] = value.decode(encoding='UTF-8',errors='strict')
    ...
    def _load_json(handler, schema):
        ...
        if not content_type.startswith('application/json'):
            raise ClientException(reason='invalid content-type')
        try:
            data = ujson.loads(handler.request.body)
        except Exception:
            raise ClientException(reason='invalid content format')
        else:
            return schema.load(data)
